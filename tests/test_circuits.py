...
from pauliopt.gates import H, S, Sdg, X, Y, Z, T, Tdg, SWAP, CX, CY, CZ, CCX, CCZ, Rx, Ry, \
    Rz, CRx, CRy, CRz
from pauliopt.utils import pi
from tests.utils import verify_equality, random_circuit

GATES_TO_TEST = [H(0), S(0), Sdg(0), X(0), Y(0), Z(0),
                 T(0), Tdg(0),
                 SWAP(0, 1), CX(0, 1), CY(0, 1), CZ(0, 1), CCX(0, 1, 2), CCZ(0, 1, 2),
                 Rx(pi / 2, 0), Ry(pi / 2, 0), Rz(pi / 2, 0),
                 CRx(pi / 2, 0, 1), CRy(pi / 2, 0, 1), CRz(pi / 2, 0, 1)]
...
class TestCircuitConstruction(unittest.TestCase):
    ...
    def test_circuit_construction(self, nr_qubits):
        ...
        self.assertTrue(verify_equality(qc, qc_),
                        "The converted circuit does not equal to original")

    @parameterized.expand([(gate.name, gate,) for gate in GATES_TO_TEST])
