...
def unpermute(circuit: QuantumCircuit):
    ...
    if circuit.metadata:
        if "initial_layout" in circuit.metadata.keys():
            ...
            circuit.compose(
                Permutation(circuit.num_qubits, perm), front=True, inplace=True
            )
            changed_circuit = True
        if "final_layout" in circuit.metadata.keys():
            circuit.compose(
                Permutation(circuit.num_qubits, circuit.metadata["final_layout"]),
                front=False,
                inplace=True,
            )
            changed_circuit = True
...
class TestPhaseCircuitSynthesis(unittest.TestCase):
    def setUp(self):
        ...
        self.circuits = [
            PhaseCircuit.random(
                self.topology.num_qubits, n_gadgets, angle_subdivision=24, rng_seed=SEED
            )
    ...
    def test_fixed_annealer(self):
        self.run_general_test(
            "naive fixed anneal",
            {
                "naive naive": lambda c: iter_anneal(
                    c,
                    self.topology,
                    self.cx_blocks,
                    1,
                    self.num_anneal_iters,
                    {
                        "phase_method": "naive",
                        "cx_method": "naive",
                        "reallocate": False,
    ...
    def test_reallocated_annealer(self):
        opts = {
            " ".join(
                [opt_kwargs["phase_method"], opt_kwargs["cx_method"]]
            ): lambda c: iter_anneal(
                c,
                self.topology,
                self.cx_blocks,
                1,
            ...
            for opt_kwargs in self.opt_kwargs_list
        }
    ...
    def test_reverse_traversal_and_anneal(self):
        opts = {
            " ".join(
                [opt_kwargs["phase_method"], opt_kwargs["cx_method"]]
            ): lambda c: reverse_traversal_anneal(
                c,
                self.topology,
                self.cx_blocks,
                self.num_rt_iters,
        ...
        for i in range(self.n_tests):
            for synth_name, optimizer in optimizers.items():
                with self.subTest(i=i, optimizer=name, synth_name=synth_name):
                    ...
                    if not Operator(synthesized).equiv(
                        Operator(self.qiskit_circuits[i])
                    ):
                        print(name)
                        print(synthesized)
