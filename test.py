...
def main(num_qubits=3):
    ...
    init_circ = pp.to_qiskit(topo)
    print(pp)
    print(init_circ)
    #print(res_circ)
    res_circ = anneal(pp.copy(), topo, nr_iterations=1000)
    print((two_qubit_count(init_circ.count_ops()) - two_qubit_count(res_circ.count_ops())) / two_qubit_count(
        init_circ.count_ops()))
    print(res_circ)
    print(verify_equality(res_circ, init_circ))
